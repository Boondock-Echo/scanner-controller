...
file_variations = {
    ...
    'run_full_qsh_discovery.py': ['runFullQSHDiscovery.py', 'run_full_qsh_discovery.py', 'runQSHDiscovery.py'],
...
with open(os.path.join(utilities_dir, '__init__.py'), 'w') as f:
    ...
    f.write("from utilities.core.shared_utils import scanner_command, clear_serial_buffer\n")
        ...
        if os.path.dirname(rel_path) == 'research':
            ...
            for subdir, file_list in files_to_move.items():
                if any(name == actual_name for _, name in file_list):
                    ...
                    break
            if target_subdir:
                f.write(f"    'utilities.research.{module_name}': 'utilities.{target_subdir}.{module_name}',\n")
        ...
        if os.path.dirname(rel_path) == '':  # Root utilities dir
            ...
            for subdir, file_list in files_to_move.items():
                if any(name == actual_name for _, name in file_list):
                    ...
                    break
            if target_subdir:
                f.write(f"    'utilities.{module_name}': 'utilities.{target_subdir}.{module_name}',\n")
