...
import logging
import sys
import scanner_controller.config
    ...
    if hasattr(adapter, 'stream_custom_search') or (
        ...
        def band_scope(ser_, adapter_, arg=""):
            ...
            if len(parts) >= 2 and parts[1].lower() == "cc":
                ...
                if action == "search":
                    try:
                        from scanner_controller.utilities.scanner.close_call_search import (
                            close_call_search,
                        )
                ...
                if action == "log":
                    try:
                        from scanner_controller.utilities.scanner.close_call_logger import (
                            record_close_calls,
                        )
