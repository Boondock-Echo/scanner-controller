...
class ScannerController:
    ...
    def _call_adapter_method(
        ...
        try:
            ...
            adapter_methods = dir(self.adapter)
            logger.debug(f"Available adapter methods: {adapter_methods}")
            ...
            # Try fallback methods for specific commands
            if snake_case_name == 'set_volume':
                # Try common alternatives for volume
                for alt_method in [
                    'volume',
                    'setVol',
                    'vol',
                    ...
                    if hasattr(self.adapter, alt_method):
                        logger.debug(
                            f"Using alternative volume method: {alt_method}"
                        )
                        method = getattr(self.adapter, alt_method)
                        return method(self.ser, *args)

                # Last resort: use send_command if available
                if hasattr(self.adapter, 'send_command'):
                    logger.debug("Using send_command as fallback for volume")
                    vol_int = int(args[0] * 100)  # Convert 0-1 to 0-100
                    return self.adapter.send_command(self.ser, f"VOL,{vol_int}")

            elif snake_case_name == 'set_squelch':
                # Try common alternatives for squelch
                for alt_method in ['squelch', 'setSql', 'sql']:
                    if hasattr(self.adapter, alt_method):
                        logger.debug(
                            f"Using alternative squelch method: {alt_method}"
                        )
                        method = getattr(self.adapter, alt_method)
                        return method(self.ser, *args)
                ...
                # Last resort: use send_command if available
                if hasattr(self.adapter, 'send_command'):
                    logger.debug("Using send_command as fallback for squelch")
                    sql_int = int(args[0] * 100)  # Convert 0-1 to 0-100
                    return self.adapter.send_command(self.ser, f"SQL,{sql_int}")
    ...
    def _send_direct_command(self, command, log_prefix="Command"):
        ...
        try:
            from scanner_gui.scanner_utils import send_command
